    ...
    def process_item(self, item, spider):
        ...
        if self.running:
            try:
                ...
                self.es.indices.create(index="%s_%s" % (self.index_current, extracted_info['date_publish'][:7]), ignore=[400, 404], body={"settings" : {"index" : {"number_of_shards":1}}})
                self.es.indices.put_mapping(index="%s_%s" % (self.index_current, extracted_info['date_publish'][:7]), doc_type='news-please', body=self.mapping)

                self.es.index(index="%s_%s" % (self.index_current, extracted_info['date_publish'][:7]), doc_type='news-please', id=ancestor,
                              body=extracted_info)
